...
const ProductCard = ({id, className, variant, ...props}) => {
  ...
  return (
    <div className={classNames('w-full cursor-pointer flex flex-col items-center justify-center relative h-auto', className)} >
        ...
        <div className='w-full' >
            <img alt={id} src={renderImages} className='rounded-xl xl:min-h-[400px] object-cover object-center' />
        </div>
        <div className="mt-3 w-full flex flex-col justify-start">
            <p className='text-text text-base max-w-xs overflow-hidden text-ellipsis whitespace-nowrap' >{variant?.name || ''}</p>
            <div className='w-full flex justify-start items-center my-1.5 text-xl font-medium' >
                ...
                <del className='ml-4 line-through text-base text-less' >₹ {attribute?.mrp}</del>
            </div>
            <div className='w-full flex justify-between items-center' >
                <div className={classNames('w-5 items-center justify-center border-text-secondary border h-5 md:w-6 md:h-6 rounded-full p-0.5')} >
                    <div style={{background: variant?.color_code}} className='w-full h-full rounded-full' ></div>
                </div>
                <div className='flex items-center justify-end' >
                    {sizes.map(size => <div className='border rounded-sm text-sm px-1 text-text-secondary min-w-6 py-0.5 ml-1 flex items-center justify-center border-text-secondary' key={size} >{size}</div>)}
                </div>
            </div>
        </div>
    </div>
