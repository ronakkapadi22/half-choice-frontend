...
const HomeBanner = () => {
  ...
  const banners = useMemo(() => home_banners || [], [home_banners]);

  useEffect(() => {
    if (home_banners?.length) {
      const links = [];

      home_banners.slice(0, 3).forEach((banner) => {
        if (banner?.image) {
        ...
        <div className="md:col-span-6 order-2 md:order-1">
          <div className="flex h-full justify-center items-center">
            <div className="w-full md:pl-8 text-center md:text-left">
              ...
              <h1 className={isActive ? "fade-up-delay text-2xl md:text-4xl !leading-normal md:!leading-[74px]" : ""}>
                {message || ""}
              </h1>
              <Button
            ...
            <div className="md:absolute mt-10 md:mt-0 rounded-full z-10 md:top-[150px] md:bottom-0 bg-white md:w-[700px] md:h-[700px] w-[300px] h-[300px]">
              <div className="relative">
                <img
                  ...
                  alt={small}
                  loading={isActive ? "eager" : "lazy"} // First image loads immediately
                  src={image}
                  style={{
